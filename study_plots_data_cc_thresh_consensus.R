...
thresh <- lapply(transpose(as.list(unique(paths_table[, .(study, cancer_type)]))), function(r) {
    if(!('data_cc_consensus.rds' %in% dir(paste0('../data/study_plots/', gsub('/', '-', r[2]), '/', r[1])))) return(NULL)
    plot_data <- readRDS(paste0('../data/study_plots/', gsub('/', '-', r[2]), '/', r[1], '/data_cc_consensus.rds'))
...
# plot(thresh[is.finite(g2m), sort(g2m)]); abline(h = 0.7)

for(r in transpose(as.list(unique(paths_table[, .(study, cancer_type)])))) {
    cat(r, '\n')
    if(!('data_cc_consensus.rds' %in% dir(paste0('../data/study_plots/', gsub('/', '-', r[2]), '/', r[1])))) next
    plot_data <- readRDS(paste0('../data/study_plots/', gsub('/', '-', r[2]), '/', r[1], '/data_cc_consensus.rds'))
    nullcond <- sapply(plot_data, function(x) ifelse(is.null(x), TRUE, all(sapply(x[names(x) != 'path'], is.null))))
    ...
    for(i in which(!nullcond)) {
        ...
        ccdata[g1s_score < 0.7 & g2m_score < 0.7, phase := 'Not cycling']
        ...
        if('data.table' %in% class(hdata)) {
            ...
            hdata <- slapply(
                ...
                function(idvec) {
                    ...
                    # Take cumulative sum of numbers of cells for each phase, to be used in ordering the cells and plotting division lines in the
                    # heatmap:
    ...
    saveRDS(plot_data, paste0('../data/study_plots/', gsub('/', '-', r[2]), '/', r[1], '/data_cc_consensus.rds'))
}
