...
study_tcga_map <- fread('../data/study_tcga_map.csv', na.strings = '', key = 'cancer_type')
...
clin_test <- lapply(cancer_types, function(ct) {
    ...
    if('Scores.csv' %in% dir(paste0('~/TCGA_data/', ct))) {
        scores <- fread(paste0('~/TCGA_data/', ct, '/Scores.csv'), key = 'sample_id')
    } else return(NULL)
    
    # Read in tumour metadata file and extract one sample ID per patient ID, according to order of priority of sample types:
    meta <- fread(paste0('~/TCGA_data/', ct, '/Cells.csv'), key = 'sample_type', na.strings = '')[
        sample_type != 'normal',
...
fwrite(clin_test_res, '../data/clin_test_res.csv')
fwrite(clin_test_data, '../data/clin_test_data.csv')
fwrite(clin_test_scores, '../data/clin_test_scores.csv')
